...
public class StudentDaoImpl implements StudentDao{
	...
	public List<BatchDTO> getAllStudentBatch() throws StudentException {
		...
		try (Connection conn=DBUtil.provideConnection()){
			...
			PreparedStatement ps=conn.prepareStatement("select s.name,s.roll,s.marks,s.email,b.bid,b.name from student s "
					+ "INNER JOIN student_batch sb ON s.roll=sb.roll INNER JOIN course c ON c.cid=sb.cid "
					+ "INNER JOIN batch b ON b.bid=c.bid;");
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
